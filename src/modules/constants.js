export const USER_HP_MIN = 60;
export const USER_HP_MAX = 80;
export const USER_STR_MIN = 5;
export const USER_STR_MAX = 9;

export const ENC_HP_MIN = 20;
export const ENC_HP_MAX = 40;
export const ENC_STR_MIN = 1;
export const ENC_STR_MAX = 12;

export const HP_METER_DURATION = 200;
export const ENC_OUT_DURATION = 500;

export const STARTING_SIZE = 7;
export const MINIMAPSIZE = 9;
export const LOS = 2;
export const PLAINWEIGHT = 2;
export const CITYFREQ = 200;
export const ENCFREQ = 10;

export const SCREEN_FADE_DURATION = 200;
